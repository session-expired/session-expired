...
CREATE INDEX IF NOT EXISTS messages_sender_sent_idx ON messages (sender_id, sent_at DESC);
CREATE INDEX IF NOT EXISTS messages_recipient_sent_idx ON messages (recipient_id, sent_at DESC);

CREATE TABLE IF NOT EXISTS lobbies (
    id BIGSERIAL PRIMARY KEY,
    host_id BIGINT NOT NULL REFERENCES users(id),
    name VARCHAR(50) NOT NULL,
    status VARCHAR(12) NOT NULL DEFAULT 'waiting' CHECK (status IN ('waiting', 'started')),
    max_players SMALLINT NOT NULL DEFAULT 4 CHECK (max_players BETWEEN 2 AND 8),
    empty_since TIMESTAMPTZ,
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP
);

ALTER TABLE lobbies ADD COLUMN IF NOT EXISTS empty_since TIMESTAMPTZ;
...
ALTER TABLE lobby_players ADD COLUMN IF NOT EXISTS selected_character VARCHAR(20);
...
CREATE UNIQUE INDEX IF NOT EXISTS lobby_players_one_active_membership ON lobby_players (user_id);
